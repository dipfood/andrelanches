...
SELECT 
  'MENU ITEMS' as tipo,
  jsonb_array_length(items->'lanches') as total_lanches,
...
-- 4. Listar resumo de produtos por categoria
SELECT 
  'LANCHES' as categoria,
  COUNT(*) as total_produtos,
  MIN((jsonb_array_elements(items->'lanches')->>'price')::decimal) as preco_min,
  MAX((jsonb_array_elements(items->'lanches')->>'price')::decimal) as preco_max
FROM menu_items 
WHERE id = 1
...
SELECT 
  'ARTESANAIS' as categoria,
  COUNT(*) as total_produtos,
  MIN((jsonb_array_elements(items->'artesanais')->>'price')::decimal) as preco_min,
  MAX((jsonb_array_elements(items->'artesanais')->>'price')::decimal) as preco_max
FROM menu_items 
WHERE id = 1
...
SELECT 
  'FRANGO' as categoria,
  COUNT(*) as total_produtos,
  MIN((jsonb_array_elements(items->'frango')->>'price')::decimal) as preco_min,
  MAX((jsonb_array_elements(items->'frango')->>'price')::decimal) as preco_max
FROM menu_items 
WHERE id = 1
...
SELECT 
  'CARNE' as categoria,
  COUNT(*) as total_produtos,
  MIN((jsonb_array_elements(items->'carne')->>'price')::decimal) as preco_min,
  MAX((jsonb_array_elements(items->'carne')->>'price')::decimal) as preco_max
FROM menu_items 
WHERE id = 1
...
SELECT 
  'HOTDOG' as categoria,
  COUNT(*) as total_produtos,
  MIN((jsonb_array_elements(items->'hotdog')->>'price')::decimal) as preco_min,
  MAX((jsonb_array_elements(items->'hotdog')->>'price')::decimal) as preco_max
FROM menu_items 
WHERE id = 1
...
SELECT 
  ...
  COUNT(*) as total_produtos,
  MIN((jsonb_array_elements(items->'bebidas')->>'price')::decimal) as preco_min,
  MAX((jsonb_array_elements(items->'bebidas')->>'price')::decimal) as preco_max
FROM menu_items 
WHERE id = 1

ORDER BY categoria;

-- 5. Listar todos os produtos para verificação final
SELECT 
  'LANCHES' as categoria,
  jsonb_array_elements(items->'lanches')->>'name' as produto,
  'R$ ' || (jsonb_array_elements(items->'lanches')->>'price')::decimal as preco
...
SELECT 
  'ARTESANAIS' as categoria,
  jsonb_array_elements(items->'artesanais')->>'name' as produto,
  'R$ ' || (jsonb_array_elements(items->'artesanais')->>'price')::decimal as preco
FROM menu_items WHERE id = 1
...
SELECT 
  'FRANGO' as categoria,
  jsonb_array_elements(items->'frango')->>'name' as produto,
  'R$ ' || (jsonb_array_elements(items->'frango')->>'price')::decimal as preco
FROM menu_items WHERE id = 1
...
SELECT 
  'CARNE' as categoria,
  jsonb_array_elements(items->'carne')->>'name' as produto,
  'R$ ' || (jsonb_array_elements(items->'carne')->>'price')::decimal as preco
FROM menu_items WHERE id = 1
...
SELECT 
  'HOTDOG' as categoria,
  jsonb_array_elements(items->'hotdog')->>'name' as produto,
  'R$ ' || (jsonb_array_elements(items->'hotdog')->>'price')::decimal as preco
FROM menu_items WHERE id = 1
...
SELECT 
  'BEBIDAS' as categoria,
  jsonb_array_elements(items->'bebidas')->>'name' as produto,
  'R$ ' || (jsonb_array_elements(items->'bebidas')->>'price')::decimal as preco
FROM menu_items WHERE id = 1

ORDER BY categoria, produto;

-- Mensagem de sucesso
SELECT 'CARDÁPIO COMPLETO ATUALIZADO COM SUCESSO!' as status,
       'Total de 40 produtos organizados em 6 categorias' as detalhes,
       'Configurações e acréscimos também atualizados' as observacao;
